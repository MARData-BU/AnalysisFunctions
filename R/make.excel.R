make.excel <- function(pathinput,fileinput,contrast,pathoutput,filename, pvalue = NULL,padj = 0.05, logFC = 1,add.colors=NULL){
  ...
  # If the contrast has only 1 element (e.g:list(c("timepoint"))) 
  ...
  color.values <- muestra[, grep(colnames(muestra), pattern = ".scaled", 
                                 fixed = TRUE)]
  wb <- createWorkbook()
  addWorksheet(wb, sheetName = "AllData")
  writeData(wb, 1, muestra)
  ...
  if (length(contrast) <= 1) {
    ...
    vector.noadj = c()
    if (is.null(pvalue)) {
      muestra.filter.adj <- muestra[, grep(colnames(muestra), 
                                           pattern = "adj.P.Val", fixed = TRUE)]
      logFC.col <- muestra[, grep(colnames(muestra), pattern = "logFC", 
                                  fixed = TRUE)]
      if (!any(muestra.filter.adj < padj)) {
        warning("PValue = 0.05 was used to filter data.")
        muestra.filter.pval <- muestra[, grep(colnames(muestra), 
                                              pattern = "P.Value", fixed = TRUE)]
        logFC.col <- muestra[, grep(colnames(muestra), 
                                    pattern = "logFC", fixed = TRUE)]
        muestra.final <- muestra[which(muestra.filter.pval <= 
                                         0.05 & abs(logFC.col) > logFC), ]
        muestra.f <- muestra.final[orderv(muestra.final[paste("FC", 
                                                              contrast[[1]][1], 
                                                              sep = ".")]), ]
        addWorksheet(wb, sheetName = paste(contrast[[1]][1], 
                                           sep = "."))
        writeData(wb, paste(contrast[[1]][1], sep = "."), muestra.f)
      }
      else {
        warning("Padj = 0.05 was used to filter data.")
        muestra.final <- muestra[which(muestra.filter.adj <= 
                                         padj & abs(logFC.col) > logFC), ]
        muestra.f <- muestra.final[orderv(muestra.final[paste("FC", 
                                                              contrast[[1]][1],
                                                              sep = ".")]), ]
        addWorksheet(wb, sheetName = paste(contrast[[1]][1], 
      ...
      logFC.col <- muestra[, grep(colnames(muestra), pattern = "logFC", 
                                  fixed = TRUE)]
      muestra.final <- muestra[which(muestra.filter.pval <= 
                                       pvalue & abs(logFC.col) > logFC), ]
      muestra.f <- muestra.final[orderv(muestra.final[paste("FC", 
                                                            contrast[[1]][1], sep = ".")]), 
      ]
      addWorksheet(wb, sheetName = paste(contrast[[1]][1], 
                                         sep = "."))
      writeData(wb, paste(contrast[[1]][1], 
                          sep = "."), muestra.f)
    }
  }
  else {
    ...
    for (i in 1:length(contrast)) {
      if (is.null(pvalue)) {
        ...
        if (!any(muestra.filter.adj < padj)) {
          ...
          logFC.col <- muestra[, grep(colnames(muestra), 
                                      pattern = "logFC", fixed = TRUE)]
          muestra.final <- muestra[which(muestra.filter.pval[i] <= 
                                           0.05 & abs(logFC.col[i]) > logFC), ]
          muestra.f <- muestra.final[orderv(muestra.final[paste("FC", 
                                                                contrast[[i]][1], 
                                                                sep = ".")]), ]
          addWorksheet(wb, sheetName = paste(contrast[[i]][1], 
                                            sep = "."))
          writeData(wb, paste(contrast[[i]][1], sep = "."), muestra.f)
        }
        else {
          warning("Padj = 0.05 was used to filter data.")
          muestra.final <- muestra[which(muestra.filter.adj[i] <= 
                                           padj & abs(logFC.col[i]) > logFC), ]
          muestra.f <- muestra.final[orderv(muestra.final[paste("FC", 
                                                                contrast[[i]][1], 
                                                                sep = ".")]), ]
          addWorksheet(wb, sheetName = paste(contrast[[i]][1], 
  ...
  for (i in 1:sheet.num) {
    ...
    addStyle(wb, sheet = i, headerStyle1, rows = 1, cols = 1:length(colnames(muestra)), 
             gridExpand = TRUE)
    bodyStyle1 <- createStyle(fontSize = 10, wrapText = TRUE, 
                              valign = "top", halign = "left")
    addStyle(wb, sheet = i, bodyStyle1, rows = 2:(length(rownames(muestra)) + 
                                                    1), cols = 1:length(colnames(muestra)), gridExpand = TRUE)
    headerStyle2 <- createStyle(fontSize = 10, halign = "center", 
                                textDecoration = "Bold", wrapText = TRUE, textRotation = 90)
    addStyle(wb, sheet = i, headerStyle2, rows = 1, cols = 1:dim(color.values)[2], 
             gridExpand = TRUE)
    NumberStyle <- createStyle(fontSize = 10, numFmt = "0.00")
    NumberStyle_adj.pval <- createStyle(fontSize = 10, numFmt = "SCIENTIFIC")
    HeatmapStyle <- createStyle(fontSize = 10, numFmt = "0")
    addStyle(wb, sheet = i, HeatmapStyle, rows = 2:(nrow(muestra) + 
                                                      1), cols = 1:(dim(color.values)[2]), gridExpand = T)
    number.col <- ncol(muestra) - dim(color.values)[2]
    FCcols <- grep("FC", colnames(muestra))
    meanCols <- grep("mean", colnames(muestra))
    adjPvalCols <- grep("adj.P.Val", colnames(muestra))
    ...
    addStyle(wb, sheet = i, NumberStyle, rows = 2:(nrow(muestra) + 
                                                     1), cols = number.col:ncol(muestra), gridExpand = T)
    addStyle(wb, sheet = i, NumberStyle, rows = 2:(nrow(muestra) + 
                                                     1), cols = FCcols, gridExpand = T)
    addStyle(wb, sheet = i, NumberStyle, rows = 2:(nrow(muestra) + 
                                                     1), cols = meanCols, gridExpand = T)
    addStyle(wb, sheet = i, NumberStyle_adj.pval, rows = 2:(nrow(muestra) + 
                                                              1), cols = adjPvalCols, gridExpand = T)
    setRowHeights(wb, sheet = i, rows = 1, heights = 150)
    setRowHeights(wb, sheet = i, rows = 2:(nrow(muestra) + 
                                             1), heights = 14)
    setColWidths(wb, sheet = i, cols = (ncol(color.values) - 
                                          1):ncol(muestra), widths = 8)
    setColWidths(wb, sheet = i, cols = 1:ncol(color.values), 
                 widths = 2)
    setColWidths(wb, sheet = i, cols = number.col:ncol(muestra), 
                 widths = 5)
    setColWidths(wb, sheet = i, cols = FCcols, widths = 5)
    setColWidths(wb, sheet = i, cols = meanCols[1]:ncol(muestra), 
                 widths = 6)
    freezePane(wb, sheet = i, firstRow = TRUE)
    if ("AffyID" %in% colnames(muestra)) {
      number.col <- which(colnames(muestra) == "AffyID")
    ...
    conditionalFormatting(wb, sheet = i, cols = 1:as.numeric(dim(color.values)[2]), 
                          rows = 1:as.numeric(dim(color.values)[1] + 1), rule = as.numeric(c(min(color.values), 
                                                                                             0, max(color.values))), style = c("blue", "white", 
                                                                                                                               "red"), type = "colorScale")
  }
  stats <- list()
  colors4stats <- c(c("#FFEA00", "#FFC000", "#00B0F0", "#92D050", 
                      "#FF6600", "#CCFF99", "#CC99FF", "#FF5252", "#5C45FF", 
                      "#45FFC7", "#fc79f4", "#00B0F0", "#9458d1", "#c2a03a", 
                      "#d1589b", "#b3a7cc", "#ccf1ff", "#1fad66", "#ffeacc", 
                      "#f0a1a1"), add.colors)
  for (i in 1:length(contrast)) {
    adjp = paste("adj.P.Val", contrast[[i]][1],
                 sep = ".")
    p = paste("P.Value", contrast[[i]][1],
              sep = ".")
    fc = paste("FC", contrast[[i]][1], 
               sep = ".")
    logfc = paste("logFC", contrast[[i]][1], 
                  sep = ".")
    stats[[i]] <- which(colnames(muestra) %in% c(adjp, p, 
                                                 fc, logfc))
    conditionalFormatting(wb, sheet = i + 1, cols = stats[[i]], 
                          rows = 1:(nrow(muestra) + 1), rule = ".", style = createStyle(bgFill = colors4stats[i], 
                                                                                        fontSize = 10), type = "contains")
    conditionalFormatting(wb, sheet = i + 1, cols = stats[[i]], 
                          rows = 1, rule = ".", style = createStyle(bgFill = colors4stats[i], 
                                                                    fontSize = 10), type = "contains")
  }
  for (i in 1:length(contrast)) {
    adjp = paste("adj.P.Val", contrast[[i]][1], 
    ...
    stats[[i]] <- which(colnames(muestra) %in% c(adjp, p, 
                                                 fc, logfc))
    conditionalFormatting(wb, sheet = 1, cols = stats[[i]], 
                          rows = 1:(nrow(muestra) + 1), rule = ".", style = createStyle(bgFill = colors4stats[i], 
                                                                                        fontSize = 10), type = "contains")
    conditionalFormatting(wb, sheet = 1, cols = stats[[i]], 
                          rows = 1, rule = ".", style = createStyle(bgFill = colors4stats[i], 
  ...
  a <- min(color.values)/5
  b <- max(color.values)/5
  vector.min <- c(min(color.values), a * 4, a * 3, a * 2, 
                  a)
  vector.max <- c(b, b * 2, b * 3, b * 4, max(color.values))
  legend.df <- as.numeric(c(vector.min, 0, vector.max))
  options(openxlsx.borderColour = "black")
  options(openxlsx.borderStyle = "thin")
  writeData(wb, "Legend", legend.df, borderStyle = getOption("openxlsx.borderStyle", 
                                                             "thin"), startCol = 1, startRow = 1, borders = "columns")
  conditionalFormatting(wb, sheet = "Legend", cols = 1, rows = 1:12, 
                        rule = as.numeric(c(min(color.values), 0, max(color.values))), 
                        style = c("blue", "white", "red"), type = "colorScale")
  saveWorkbook(wb, file.path(pathoutput, paste(filename, "xlsx", 
                                               sep = ".")), overwrite = TRUE)
  message("The function was performed successfully")
  }
}
